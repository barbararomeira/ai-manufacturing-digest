...
"""
    ...
    try:
        ...
        return {
            ...
            "category": [str(t)[:50] for t in (data.get("category") or []) if t],
            "industry": [str(i)[:50] for i in (data.get("industry") or []) if i] or ["General"]
        }
    except Exception as e:
...
def post_to_notion(title, problem, ai_solution, category, industry, source, date_str):
    ...
    payload = {
        ...
        "properties": {
            ...
            "Category": {"multi_select": [{"name": t} for t in category[:5]]},
    ...
    for feed_url in FEEDS:
        ...
        try:
            ...
            for entry in feed.entries[:MAX_ARTICLES_PER_FEED]:
                ...
                text = clean_html(entry.get("summary", "") + " " + entry.get("content", [{}])[0].get("value", ""))
                ...
                if use_case:
                    post_to_notion(
                        title, use_case["problem"], use_case["ai_solution"],
                        use_case["category"], use_case["industry"], entry.link, date_str
                    )
                    time.sleep(1.5)
                else:
